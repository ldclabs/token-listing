...
build-wasm:
	cargo build --release --target wasm32-unknown-unknown -p ic_auction -p token_listing_canister -p tokens_canister -p tokens_canister -p images_canister
...
build-did:
	...
	candid-extractor target/wasm32-unknown-unknown/release/images_canister.wasm > src/images_canister/images_canister.did
	dfx generate

gzip-wasm:
	@set -e; \
	for CAN in ic_auction token_listing_canister tokens_canister images_canister; do \
